            ...
            while position < len(line):
                ...
                for tokenRegex in regexExpressions:
                    ...
                    if match:
                        data = match.group(0)
                        if tag:
                            token = TT.token(tag, data, [lineNumber, position])
...
def stringFichier(path):
    with open(path, 'r') as ff:
        return "".join(ff.readlines())

if __name__ == "__main__":
    Lex = Lexer()
    Lex.lex(stringFichier("./main.c"))
    list(map(print, Lex.tokens))
